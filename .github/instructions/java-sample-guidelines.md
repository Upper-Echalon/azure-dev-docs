...
- Prefer built-in JDK functionality over third-party libraries when the built-in approach is reasonable. For example, prefer `java.net.http.HttpClient` over Apache HttpClient, and prefer `SearchDocument` (dynamic map) over typed POJOs with Jackson annotations when full type safety isn't needed.
...
- Avoid using the `exec-maven-plugin` dependency. Use Maven commands directly instead.
...
- Use 4-space indentation. Don't use 8-space indentation.
...
- Target Java 21+ syntax conventions for new content: prefer text blocks (triple-quote `"""` for multi-line strings), `var` for local type inference when the type is obvious, and pattern matching. Older content targeting earlier Java versions is acceptable on a case-by-case basis — if a PR updates an article that targets a pre-21 version, flag it as a suggestion to consider updating to Java 21+, but don't block the PR on it.
...
- Use `System.out.printf()` with `%n` for line terminators rather than `System.out.println()` with `.formatted()` string concatenation. `printf` is more readable for complex formatted output.
